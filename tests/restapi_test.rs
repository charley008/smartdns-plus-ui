...
use nix::libc::c_char;
use smartdns_plus_ui::{http_api_msg, http_jwt::JwtClaims, smartdns::LogLevel};
use std::ffi::CString;
...
fn test_rest_api_server_version() {
    ...
    assert_eq!(version.0, smartdns_plus_ui::smartdns::smartdns_version());
...
fn test_rest_api_server_status() {
    ...
    unsafe {
        let server_type = smartdns_plus_ui::smartdns::smartdns_c::dns_server_type_t_DNS_SERVER_UDP;
        let mut flags: smartdns_plus_ui::smartdns::smartdns_c::client_dns_server_flags =
            std::mem::zeroed();
        let ip = CString::new("1.2.3.4").expect("CString::new failed");
        let port = 3353;
        smartdns_plus_ui::smartdns::smartdns_c::dns_client_add_server(
