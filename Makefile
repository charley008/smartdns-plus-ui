...
BIN=smartdns-plus-ui
PREFIX := /usr
SBINDIR := $(PREFIX)/sbin
SLIBDIR := $(PREFIX)/lib
DESTDIR :=
CARGO_BUILD_ENV :=
SMARTDNS_INCLUDE_DIR ?= $(CURDIR)/vendor/include
SMARTDNS_TEST_LIB ?=
...
test-prepare:
	@if [ -z "$(SMARTDNS_TEST_LIB)" ]; then \
		echo "SMARTDNS_TEST_LIB is not set."; \
		echo "Standalone build does not require SmartDNS source, but tests need a prepared libsmartdns-test.a."; \
		echo "Example:"; \
		echo "  make test SMARTDNS_TEST_LIB=/path/to/libsmartdns-test.a"; \
		exit 1; \
	fi

$(BIN):
	CXXFLAGS= CFLAGS= MAKEFLAGS= RUSTFLAGS="$(CARGO_RUSTFLAGS) $(RUSTFLAGS)" $(CARGO_BUILD_ENV) SMARTDNS_INCLUDE_DIR="$(SMARTDNS_INCLUDE_DIR)" cargo build $(CARGO_BUILD_ARGS) --features "build-release"
	cp $(CARGO_BUILD_PATH)/libsmartdns_plus_ui.so target/smartdns_plus_ui.so

install: $(BIN)
	install -v -m 0644 -D -t $(DESTDIR)$(SLIBDIR)/smartdns target/smartdns_plus_ui.so

test: test-prepare
	MAKEFLAGS= WITH_ZLIB=0 SMARTDNS_INCLUDE_DIR="$(SMARTDNS_INCLUDE_DIR)" SMARTDNS_TEST_LIB="$(SMARTDNS_TEST_LIB)" cargo test

clean:
	cargo clean
	rm -rf target/smartdns_plus_ui.so
