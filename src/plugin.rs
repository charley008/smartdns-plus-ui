...
impl SmartdnsPlugin {
    ...
    fn parser_args(&self, args: &Vec<String>) -> Result<(), Box<dyn Error>> {
        ...
        opts.optopt("", "config-file", "smartdns config file", "PATH");
        opts.optopt("", "rules-dir", "smartdns rules dir", "PATH");
        if args.len() <= 0 {
            return Ok(());
        }
        ...
        let www_root = Plugin::dns_conf_plugin_config("smartdns-plus-ui.www-root");
        if let Some(www_root) = www_root {
            http_conf.http_root = smartdns_conf_get_conf_fullpath(&www_root);
        }

        let config_file = Plugin::dns_conf_plugin_config("smartdns-plus-ui.config-file");
        if let Some(config_file) = config_file {
            http_conf.config_file = smartdns_conf_get_conf_fullpath(&config_file);
        }

        let rules_dir = Plugin::dns_conf_plugin_config("smartdns-plus-ui.rules-dir");
        if let Some(rules_dir) = rules_dir {
            http_conf.rules_dir = smartdns_conf_get_conf_fullpath(&rules_dir);
        }

        let ip = Plugin::dns_conf_plugin_config("smartdns-plus-ui.ip");
        if let Some(ip) = ip {
        ...
        if let Some(config_file) = matches.opt_str("config-file") {
            http_conf.config_file = config_file;
        }

        if let Some(rules_dir) = matches.opt_str("rules-dir") {
            http_conf.rules_dir = rules_dir;
        }

        let mut token_expire = Plugin::dns_conf_plugin_config("smartdns-plus-ui.token-expire");
    ...
    pub fn start(&self, args: &Vec<String>) -> Result<(), Box<dyn Error>> {
        dns_log!(LogLevel::INFO, "start smartdns-plus-ui server.");
