...
impl HttpServerConfig {
    ...
    pub fn new() -> Self {
        ...
        HttpServerConfig {
            ...
            rules_dir: HTTP_SERVER_DEFAULT_RULES_DIR.to_string(),
        }
    }
    ...
    pub fn settings_map(&self) -> std::collections::HashMap<String, String> {
        ...
        map.insert("config_file".to_string(), self.config_file.clone());
        map.insert("rules_dir".to_string(), self.rules_dir.clone());
        map
    }

    pub fn load_config(&mut self, data_server: Arc<DataServer>) -> Result<(), Box<dyn Error>> {
        if let Some(password) = data_server.get_config("smartdns-plus-ui.password") {
            self.password = password;
        } else {
            if let Some(password_from_file) =
                data_server.get_server_config_from_file("smartdns-plus-ui.password")
            {
                self.password =
        ...
        if let Some(config_file) = data_server.get_server_config("smartdns-plus-ui.config-file") {
            self.config_file = config_ui::config_file_from_settings(Some(config_file));
        }

        if let Some(rules_dir) = data_server.get_server_config("smartdns-plus-ui.rules-dir") {
            self.rules_dir = config_ui::rules_dir_from_settings(Some(rules_dir));
        }
...
impl HttpServer {
    ...
    pub fn get_mime_type(&self, file: &str) -> String {
        let ext = file.split('.').last().unwrap_or("txt");
    ...
    async fn server_handle_http_request(
        ...
        match read(filepath).await {
            ...
            Err(_) => {
                let body = if uri_path == "/" || uri_path == "/index.html" {
                    include_str!("../wwwroot/index.html").to_string()
                } else if uri_path == "/app.js" {
                    include_str!("../wwwroot/app.js").to_string()
                } else if uri_path == "/style.css" {
                    include_str!("../wwwroot/style.css").to_string()
                } else {
                    "Page Not Found".to_string()
                };
                let mut response = Response::new(Full::new(Bytes::from(body)));
                if uri_path == "/" || uri_path == "/index.html" {
                    response
                        .headers_mut()
