...
impl API {
    ...
    pub fn new() -> Self {
        ...
        api.register(Method::GET, "/api/plus/config", true, APIRoute!(API::api_plus_get_config));
        api.register(Method::POST, "/api/plus/save", true, APIRoute!(API::api_plus_save));
        api.register(Method::POST, "/api/plus/save-and-restart", true, APIRoute!(API::api_plus_save_and_restart));
        api.register(Method::GET, "/api/plus/backups", true, APIRoute!(API::api_plus_backups));
        api.register(Method::POST, "/api/plus/restore", true, APIRoute!(API::api_plus_restore));
        api.register(Method::GET, "/api/plus/templates", true, APIRoute!(API::api_plus_templates));
        api.register(Method::GET, "/api/plus/rule-sets/{name}", true, APIRoute!(API::api_plus_rule_set_get));
        api.register(Method::POST, "/api/plus/rule-sets/{name}", true, APIRoute!(API::api_plus_rule_set_save));
        api.register(Method::GET, "/api/plus/managed/assets", true, APIRoute!(API::api_plus_managed_assets_get));
        api.register(Method::POST, "/api/plus/managed/assets", true, APIRoute!(API::api_plus_managed_assets_save));
    ...
    async fn api_plus_get_config(
        this: Arc<HttpServer>,
        _param: APIRouteParam,
        _req: Request<body::Incoming>,
    ) -> Result<Response<Full<Bytes>>, HttpError> {
        let conf = this.get_conf();
        let overview = config_ui::read_overview(&conf.config_file, &conf.rules_dir)?;
        API::response_build(
            StatusCode::OK,
    ...
    async fn api_plus_save_and_restart(
        this: Arc<HttpServer>,
        _param: APIRouteParam,
        req: Request<body::Incoming>,
    ) -> Result<Response<Full<Bytes>>, HttpError> {
    ...
    async fn api_plus_backups(
        this: Arc<HttpServer>,
        _param: APIRouteParam,
        _req: Request<body::Incoming>,
    ) -> Result<Response<Full<Bytes>>, HttpError> {
        let conf = this.get_conf();
        let backups = config_ui::list_backups(&conf.config_file)?;
        API::response_build(
            StatusCode::OK,
    ...
    async fn api_plus_restore(
        ...
        let conf = this.get_conf();
        let whole_body = String::from_utf8(req.into_body().collect().await?.to_bytes().into())?;
        let payload: serde_json::Value = serde_json::from_str(&whole_body)
            .map_err(|e| HttpError::new(StatusCode::BAD_REQUEST, e.to_string()))?;
        let path = payload["path"]
            .as_str()
            .ok_or_else(|| HttpError::new(StatusCode::BAD_REQUEST, "path not found".to_string()))?;
        config_ui::restore_backup(&conf.config_file, path)?;
        API::response_build(
            StatusCode::OK,
            json!({
                "restored": true,
    ...
    async fn api_plus_templates(
        ...
        API::response_build(
            StatusCode::OK,
            json!({
                "beginner": config_ui::default_beginner_config(),
                "rules_text": config_ui::default_rules_text(),
            })
            .to_string(),
        )
    }

    async fn api_plus_rule_set_get(
        this: Arc<HttpServer>,
        param: APIRouteParam,
        req: Request<body::Incoming>,
    ) -> Result<Response<Full<Bytes>>, HttpError> {
        ...
        let detail = config_ui::read_rule_set_detail(&conf.config_file, name, query, offset, limit)
            .map_err(|e| HttpError::new(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

        let Some(detail) = detail else {
            return API::response_error(StatusCode::NOT_FOUND, "Rule set not found.");
        };
    ...
    async fn api_plus_rule_set_save(
        this: Arc<HttpServer>,
        param: APIRouteParam,
        req: Request<body::Incoming>,
    ) -> Result<Response<Full<Bytes>>, HttpError> {
    ...
    async fn api_plus_managed_assets_get(
        this: Arc<HttpServer>,
        _param: APIRouteParam,
        _req: Request<body::Incoming>,
    ) -> Result<Response<Full<Bytes>>, HttpError> {
        ...
        API::response_build(
            StatusCode::OK,
            serde_json::to_string(&overview.managed_assets)
                .map_err(|e| HttpError::new(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?,
        )
    }

    async fn api_plus_apply(
        this: Arc<HttpServer>,
        _param: APIRouteParam,
        _req: Request<body::Incoming>,
    ) -> Result<Response<Full<Bytes>>, HttpError> {
        let conf = this.get_conf();
        config_ui::apply_working_config(&conf.config_file, &conf.rules_dir)?;
        Plugin::smartdns_restart();
        API::response_build(StatusCode::OK, json!({"applied":true}).to_string())
    }

    async fn api_plus_managed_assets_save(
        this: Arc<HttpServer>,
        _param: APIRouteParam,
        req: Request<body::Incoming>,
    ) -> Result<Response<Full<Bytes>>, HttpError> {
        ...
        let save_req: config_ui::SaveManagedAssetsRequest = serde_json::from_str(&whole_body)
            .map_err(|e| HttpError::new(StatusCode::BAD_REQUEST, e.to_string()))?;
        let validation =
            config_ui::save_managed_assets(&conf.config_file, &conf.rules_dir, &save_req)?;
        API::response_build(
            StatusCode::OK,
            json!({
                "saved": validation.ok,
